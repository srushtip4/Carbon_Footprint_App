  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="bg-white/90 backdrop-blur-sm rounded-xl p-5 border border-emerald-100 shadow-sm"><p className="text-sm text-gray-500">Completed Actions</p><p className="text-2xl font-bold text-emerald-700">{completedCount}/{actions.length}</p></div>
        <div className="bg-white/90 backdrop-blur-sm rounded-xl p-5 border border-emerald-100 shadow-sm"><p className="text-sm text-gray-500">CO2 Savings from Actions</p><p className="text-2xl font-bold text-emerald-700">{totalSavings.toFixed(1)} kg</p></div>
      </div>
      <div className="space-y-3 mb-6">
        {actions.map(action => {
          const Icon = CATEGORY_ICONS[action.category] ?? Leaf;
          return (
            <div key={action.id} className={`bg-white/90 backdrop-blur-sm rounded-xl p-4 border transition-all flex items-start gap-4 ${action.completed ? 'border-emerald-200 bg-emerald-50/50' : 'border-gray-100'} ${justCompleted === action.id ? 'ring-2 ring-emerald-400 scale-[1.02]' : ''}`}>
              <button onClick={() => toggleAction(action)} className={`mt-0.5 flex-shrink-0 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${action.completed ? 'border-emerald-500 bg-emerald-500' : 'border-gray-300 hover:border-emerald-400'}`}>
                {action.completed && <Check className="w-4 h-4 text-white" />}
              </button>
              <div className="flex-1">
                <div className="flex items-center gap-2"><Icon className="w-4 h-4 text-gray-400" /><span className={`font-semibold text-sm ${action.completed ? 'text-gray-400 line-through' : 'text-gray-800'}`}>{action.title}</span></div>
      ...
      <div className="relative">
        <button onClick={() => setShowAddMenu(!showAddMenu)} className="flex items-center gap-2 px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-lg transition shadow-sm"><Plus className="w-4 h-4" /> Add Action</button>
        {showAddMenu && (
          <div className="absolute top-full mt-2 left-0 right-0 bg-white rounded-xl shadow-xl border border-gray-100 p-4 z-20 max-h-80 overflow-y-auto">
            {Object.entries(ACTION_TEMPLATES).map(([cat, templates]) => (
              <div key={cat} className="mb-4">
                <h3 className="text-xs font-bold text-gray-400 uppercase mb-2">{cat}</h3>
                {templates.map((t, idx) => (
                  <button key={idx} onClick={() => addAction(t)} className="w-full text-left flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-emerald-50 transition text-sm">
                    <Plus className="w-3 h-3 text-emerald-500" /><span className="text-gray-700 flex-1">{t.title}</span><span className="text-xs text-emerald-600">-{t.co2SavingKg} kg</span>
                  </button>
                ))}
              </div>
            ))}
