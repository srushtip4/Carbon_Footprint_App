...
export default function GardenPage() {
  ...
  if (!user || !baselineEmissions) {
    return (<div className="max-w-3xl mx-auto px-4 py-12 text-center"><TreePine className="w-12 h-12 text-emerald-400 mx-auto mb-4" /><h2 className="text-xl font-bold text-gray-800">Complete Your Quiz to Unlock</h2><p className="text-gray-500">Your eco-garden grows as you reduce your carbon footprint.</p></div>);
  }
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <div className="flex items-center gap-3 mb-6"><TreePine className="w-6 h-6 text-emerald-600" /><h1 className="text-2xl font-bold text-gray-900">Your Eco-Garden</h1></div>
      <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg p-6 border border-emerald-100 mb-6">
        <VirtualGarden level={gardenLevel} wilting={wilting} />
        <div className="mt-4 text-center">
          <p className="text-lg font-bold text-gray-900">{wilting ? 'Your tree is wilting...' : phases[gardenLevel - 1].name}</p>
      ...
      <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg p-6 border border-emerald-100">
        <div className="flex items-center gap-2 mb-4"><Info className="w-5 h-5 text-emerald-600" /><h2 className="text-lg font-bold text-gray-900">Growth Phases</h2></div>
        <div className="space-y-3">
          {phases.map(phase => (
            <div key={phase.level} className={`flex items-center gap-4 p-3 rounded-xl transition ${gardenLevel === phase.level ? 'bg-emerald-50 border border-emerald-200' : 'bg-gray-50/50'}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${gardenLevel >= phase.level ? 'bg-emerald-500 text-white' : 'bg-gray-200 text-gray-500'}`}>
